 ...
 *--------------------------------------------------------------------
 * $Id: Barcode1DBarCode.class.php,v 1.2 2008/07/10 04:23:26 jsgoupil Exp $
 *--------------------------------------------------------------------
 * Copyright (C) Jean-Sebastien Goupil
 * http://www.barcodephp.com
 */
namespace Common\BarCode;
use Common\BarCode\Barcode;
use Common\BarCode\FontBarCode;

abstract class Barcode1DBarCode extends BarcodeBarCode
{
    ...
    {
        if($font instanceof FontBarCode) {
    ...
    public function getMaxSize()
    {
        ...
        $textHeight = 0;
        if(!empty($label)) {
            if($this->textfont instanceof FontBarCode) {
    ...
    /**
     * Draws a BarCode of $color depending of the resolution
     * @param resource $img
     * @param FColor   $color
     */
    ...
    /**
     * Draws the label under the barcode.bak
     * @param resource $im
     */
    protected function drawText($im)
    {
        ...
        if(!empty($label)) {
            $pA = $this->getMaxSize();
            $pB = Barcode1DBarCode::getMaxSize();
            $w  = $pA[0] - $pB[0];
            if($this->textfont instanceof FontBarCode) {
                $textfont = clone $this->textfont;
                $textfont->setText($label);
